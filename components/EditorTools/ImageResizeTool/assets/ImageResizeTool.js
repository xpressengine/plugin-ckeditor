	...
	events: {
		iconClick: function(targetEditor, cbAppendToolContent) {
			if(targetEditor.config.perms.upload) {
				...
				var cWindow = window.open(imageResizeURL.get('popup'), 'createImageResizePopup', "width=850,height=970,directories=no,titlebar=no,toolbar=no,location=no,status=no,menubar=no,scrollbars=no,resizable=no");
