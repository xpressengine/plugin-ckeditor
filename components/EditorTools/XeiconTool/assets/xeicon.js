window.XEeditor.tools.define({
  ...
  events: {
    iconClick: function (targetEditor, cbAppendToolContent) {
      var cWindow = window.open(xeiconToolURL.get('popup'), 'createPopup', 'width=980,height=600,directories=no,titlebar=no,toolbar=no,location=no,status=no,menubar=no,scrollbars=yes,resizable=no')
    ...
    elementDoubleClick: function () {
      ...
      var cWindow = window.open(xeiconToolURL.get('edit_popup'), 'editPopup', 'width=980,height=600,directories=no,titlebar=no,toolbar=no,location=no,status=no,menubar=no,scrollbars=yes,resizable=no')
